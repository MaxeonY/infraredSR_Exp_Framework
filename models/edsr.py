...
class ResidualBlock(nn.Module):
    def __init__(self, n_feats: int = 64, res_scale: float = 0.1) -> None:
        super().__init__()
...
class UpsampleBlock(nn.Module):
    def __init__(self, scale: int, n_feats: int) -> None:
        super().__init__()
        if scale not in (2, 4):
            raise ValueError(f"EDSR currently supports scale=2/4, got {scale}")
...
class EDSR(nn.Module):
    def __init__(
        ...
        body = [ResidualBlock(n_feats=n_feats, res_scale=res_scale) for _ in range(n_resblocks)]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 4:
            raise ValueError(f"EDSR expects [B, C, H, W], got {tuple(x.shape)}")
        if x.shape[1] != self.in_channels:
            raise ValueError(
                f"EDSR expects {self.in_channels} channels, got {x.shape[1]}"
            )
